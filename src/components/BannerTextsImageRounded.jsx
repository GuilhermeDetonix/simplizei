const BannerTextsImageRounded = ({ title, subTitle = '', backgroundColor, backgroundImage, textColor, buttons = [] }) => {
    return (
        <section className={`relative h-[400px] md:h-[420px] overflow-hidden ${backgroundColor || 'bg-white'} rounded-bl-[20%]  rounded-br-[20%]`}>
                    ...
                    <div className="mt-8 flex flex-wrap gap-4">
                        {
                            buttons.map((button, index) => (
                                <button key={index} className={`${button.class} ${button.classHover} text-white font-semibold py-3 px-6 rounded-full transition shadow-md`} onClick={button.onClick}>
                                    {button.text}
                                </button>
                            ))
                        }
                ...
                <div className="relative z-10 hidden lg:block">
                    <div className="w-[600px] h-[450px] rounded-l-[30%] overflow-hidden ml-auto">
                        <img
                            ...
                            className="rounded-l-[500px] w-full h-full object-cover object-center"
                            lazyLoading="lazy"
                        />
                    </div>
                </div>
...
export default BannerTextsImageRounded;